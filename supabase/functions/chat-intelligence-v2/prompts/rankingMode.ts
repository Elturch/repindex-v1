...
REGLA 17 — COLUMNA TENDENCIA OBLIGATORIA EN LA TABLA DE RANKING: inmediatamente DESPUÉS de copiar el bloque <PRE_RENDERED_RANKING_TABLE>, añade en la MISMA sección una segunda tabla markdown enriquecida con la columna TENDENCIA. Estructura exacta de columnas: # | EMPRESA | RIX | TENDENCIA | OBS. La columna TENDENCIA debe contener SOLO un símbolo (sin texto): ↑ si (RIX última semana − RIX primera semana) ≥ +3, ↓ si ≤ −3, → si está entre −2 y +2. El cálculo usa la serie semanal del DataPack (campos por empresa con desglose por semana). Si una empresa no tiene al menos 2 semanas con dato, usa "n/d" en la celda TENDENCIA. NO toques la tabla pre-renderizada original (la conservas literal arriba); esta tabla enriquecida va justo debajo, con un breve epígrafe "Tendencia semana inicial → semana final:" antes de ella. NO añadas la columna TENDENCIA a la tabla de desglose semanal de otras secciones.
