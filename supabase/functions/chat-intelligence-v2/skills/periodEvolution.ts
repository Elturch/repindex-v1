...
export const periodEvolutionSkill: Skill = {
  ...
  async execute(input: SkillInput): Promise<SkillOutput> {
    ...
    const systemPrompt = [
      ...
      "ANÁLISIS CAUSAL DE VARIACIONES (obligatorio): cuando detectes variaciones semana-a-semana superiores a 10 puntos en el RIX de la empresa, NO te limites a reportar el delta. Formula hipótesis causales basadas EXCLUSIVAMENTE en los datos disponibles en el datapack: (a) qué submétrica (NVM/DRM/SIM/CEM/RMM/GAM/DCM/CXM) lideró el movimiento, (b) qué modelo o modelos de IA arrastraron el cambio (si la divergencia inter-modelo se amplió o se cerró), (c) si el datapack contiene fuentes/noticias asociadas a esa semana, menciónalas como hipótesis explicativa. CITAS CONCRETAS OBLIGATORIAS: cuando formules una hipótesis causal, DEBES citar noticias o eventos concretos del datapack (URL, fecha, titular o medio) que coincidan temporalmente con la subida o bajada. PROHIBIDO usar hipótesis vagas tipo 'ausencia de novedades', 'falta de cobertura mediática' o 'entorno tranquilo' sin respaldo en el datapack. PROHIBIDO inventar eventos o noticias no presentes en el datapack. Si no hay fuentes en el datapack que expliquen la variación en esa semana, dilo explícitamente con esta fórmula: 'variación >10 pts sin causa atribuible en el datapack (no hay fuentes citadas para la semana YYYY-MM-DD)'. NUNCA dejes una variación >10 pts sin párrafo explicativo dedicado.",
    ].filter(Boolean).join("\n\n");
