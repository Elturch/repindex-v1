...
function buildUserMessage(question: string, datapack: DataPack): string {
  ...
  const tables = datapack.pre_rendered_tables.length
    ? [
      ...
      "## 3. Visión por Modelo de IA — OBLIGATORIO ≥120 palabras de narrativa interpretativa (NO sólo ranking). DEBE incluir: (i) qué modelo es más optimista y por qué (ej: 'Gemini puntúa más alto en NVM porque pondera notas de prensa positivas'); (ii) qué modelo es más crítico y en qué dimensión concreta — usa la tabla 'Divergencia inter-modelo POR DIMENSIÓN' para identificar el modelo + alto y + bajo en CADA métrica (NVM/SIM/DRM/RMM/CEM/GAM/DCM/CXM); (iii) cita explícitamente la dimensión con MAYOR desacuerdo (la primera fila de esa tabla, σ más alto) y la de MAYOR consenso (la última, σ más bajo), explicando qué significa cada gap; (iv) si el consenso global es alto/medio/bajo y qué implica; (v) tabla [MODEL_BREAKDOWN_TABLE] copiada literal; (vi) conclusión 1-2 frases sobre fiabilidad del consenso. NO basta con listar puntuaciones.",
      "## 4. Evolución Temporal — OBLIGATORIO ≥100 palabras. DEBE incluir: (i) tendencia neta del período (subida/bajada/lateral) con cifra Δ exacta y un INDICADOR DE TENDENCIA junto al nombre de la empresa calculado así: comparar primera vs última semana del periodo — si sube >3 pts ↑, si baja >3 pts ↓, si la diferencia ≤3 pts →; (ii) identificación de 1-2 semanas con cambio brusco y posible disparador (si lo hay en las fuentes); (iii) tabla [TEMPORAL_EVOLUTION_TABLE]; (iv) conclusión sobre estabilidad/volatilidad y qué KPI lidera el cambio.",
      ...
      "REGLA 18 — TABLA HTML DE 8 SUBMÉTRICAS POR EMPRESA: en la sección 3 ('Análisis empresa por empresa' — o, si esta skill se usa para una única empresa, justo DESPUÉS de la narrativa de la sección 6) DEBES generar, para CADA empresa analizada y DESPUÉS de su párrafo narrativo (nunca antes), una tabla HTML compacta con las 8 submétricas canónicas. Estructura EXACTA: <table style=\"font-size:0.85em;border-collapse:collapse;margin:8px 0\"><thead><tr><th style=\"border-bottom:1px solid #ccc;padding:4px 8px;text-align:left\">Métrica</th><th style=\"border-bottom:1px solid #ccc;padding:4px 8px\">Valor</th><th style=\"border-bottom:1px solid #ccc;padding:4px 8px\">Peso</th><th style=\"border-bottom:1px solid #ccc;padding:4px 8px\">Contribución</th><th style=\"border-bottom:1px solid #ccc;padding:4px 8px\">Banda</th></tr></thead><tbody>...</tbody></table>. Una fila por cada métrica EN ESTE ORDEN: CEM, SIM, RMM, DRM, GAM, CXM, DCM, NVM. Banda según el valor: 🔴 (<40), 🟠 (40–54), 🟡 (55–69), 🟢 (70–84), 💎 (≥85). Si una métrica no está disponible en el datapack, escribe 'n/d' en Valor / Peso / Contribución / Banda — está PROHIBIDO inventar valores. Tabla SIN bordes gruesos, fuente pequeña (font-size:0.85em). NO la sustituyas por markdown ni la coloques antes de la narrativa.",
      "REGLA 20 — NOMBRES COMPLETOS DE MÉTRICAS EN EL ANÁLISIS POR EMPRESA: cuando menciones una submétrica por PRIMERA VEZ dentro del análisis narrativo de cada empresa, usa SIEMPRE el formato 'Nombre completo (ACRÓNIMO) valor'. Ejemplo correcto: 'Gestión de Controversias (CEM) 89'. Ejemplo INCORRECTO: 'CEM 89'. Diccionario obligatorio: NVM=Calidad de la Narrativa, DRM=Fortaleza de Evidencia, SIM=Autoridad de Fuentes, RMM=Actualidad y Empuje Mediático, CEM=Gestión de Controversias, GAM=Percepción de Gobernanza, DCM=Coherencia Informativa, CXM=Ejecución Corporativa, RIX=Índice de Reputación Algorítmica. Menciones posteriores de la MISMA métrica DENTRO del bloque narrativo de la MISMA empresa pueden usar solo el acrónimo. Al pasar a otra empresa, vuelve a usar el nombre completo en la primera mención.",
      ...
      "## 8. Fuentes citadas por los modelos de IA — IMPRESCINDIBLE incluir esta sección con el header EXACTO `## 8. Fuentes citadas por los modelos de IA`. Escribe SOLO un párrafo introductorio (2-3 frases) usando los totales del 'Resumen de fuentes citadas' que aparece más abajo (cuántas URLs únicas, cuántos medios, qué dominios dominan). Termina la sección con la línea EXACTA `<!--CITEDSOURCESHERE-->` en su propia línea y NADA más después. NO intentes listar las URLs individuales: el sistema sustituirá ese marcador por un resumen top-30 dominios (la lista exhaustiva vive en el Anexo final). Si omites esta sección o el marcador, el informe quedará incompleto.",
      "## 9. Ficha Metodológica + Cierre Ejecutivo — esta sección tiene DOS partes OBLIGATORIAS, en este orden:\n  PARTE A — Ficha de datos (1 párrafo seco + tabla de divergencia por dimensión): período (declarando solicitado vs disponible si difieren), modelos efectivamente usados, observaciones totales, semanas únicas, divergencia agregada del RIX (incluye literalmente [DIVERGENCE_BLOCK]) y, A CONTINUACIÓN, copia LITERALMENTE el bloque [DIMENSION_DIVERGENCE_BLOCK] (tabla NVM/SIM/DRM/RMM/CEM/GAM/DCM/CXM con σ y rango por modelo). NO regeneres la tabla.\n  PARTE B — Cierre ejecutivo OBLIGATORIO (1 párrafo separado de 3-4 frases, tono ejecutivo, SIEMPRE presente incluso si no hay divergencias): debe sintetizar (a) robustez del dataset y nivel de confianza del informe (apoyándote en qué dimensiones tienen consenso alto vs bajo según la tabla por dimensión), (b) UNA recomendación accionable de máxima prioridad para esta empresa derivada del análisis (con métrica + acción + horizonte), (c) próximo barrido recomendado o señal a vigilar. PROHIBIDO omitir esta parte B; PROHIBIDO fundirla con la ficha en el mismo párrafo.",
      "",
