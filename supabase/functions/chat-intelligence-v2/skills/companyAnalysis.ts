...
function buildUserMessage(question: string, datapack: DataPack): string {
  ...
  const tables = datapack.pre_rendered_tables.length
    ? [
      ...
      "## 3. Visión por Modelo de IA — OBLIGATORIO ≥120 palabras de narrativa interpretativa (NO sólo ranking). DEBE incluir: (i) qué modelo es más optimista y por qué (ej: 'Gemini puntúa más alto en NVM porque pondera notas de prensa positivas'); (ii) qué modelo es más crítico y en qué dimensión concreta; (iii) si el consenso es alto/medio/bajo y qué implica; (iv) tabla [MODEL_BREAKDOWN_TABLE] copiada literal; (v) conclusión 1-2 frases sobre fiabilidad del consenso. NO basta con listar puntuaciones.",
