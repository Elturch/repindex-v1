...
function extractRequestedSubsector(question: string): string | null {
  ...
  if (!m) return null;
  // Trim trailing report clauses without cutting valid labels such as
  // "Banca de Inversión / Gestión". Temporal clauses generated by the
  // report builder start with "entre", "desde", "hasta", "con", etc.
  const raw = m[1]
    .replace(/\s+\b(entre|desde|hasta|en|con|seg[uú]n|multi[-\s]?modelo|usando|como|priorizando|limitado|durante|para|sobre|últimas?|ultimas?)\b.*$/i, "")
    .replace(/[\s.·:;,–—-]+$/g, "")
    .trim();
