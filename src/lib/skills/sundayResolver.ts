// SYNC WITH: supabase/functions/_shared/sundayResolver.ts — drift entre
// estas dos implementaciones = bug. Si modificas una, modifica la otra y
// actualiza los tests del Diff 5.
...
export async function resolveLastClosedSunday(supabase: SupabaseClient, now: Date = new Date()): Promise<ResolvedSunday> {
  ...
  try {
    ...
    if (!error && Array.isArray(data) && data.length > 0) {
      ...
      for (const r of data) {
        const d = String((r as any)["07_period_to"] ?? "").slice(0, 10);
